...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $guests = $_POST['guests'];
    $additional_guest = isset($_POST['additional_guest']) ? $_POST['additional_guest'] : NULL;
    $total_cost = $_POST['total_cost'];
    $deposit_amount = $_POST['deposit_amount'];
    $remaining_balance = $_POST['remaining_balance'];
    $payment_method = $_POST['payment_method'];

    if (empty($first_name) || empty($last_name) || empty($contact_number) || empty($email) || empty($room) || empty($check_in_date) || empty($check_out_date) || empty($guests) || empty($payment_method)) {
    ...
    $stmt = $conn->prepare("INSERT INTO pending_room_reservations (first_name, middle_name, last_name, contact_number, email, handle, sources, source_other, room, check_in_date, check_out_date, check_in_time, check_out_time, guests, additional_guest, total_cost, deposit_amount, remaining_balance, payment_method, proof_of_payment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("sssisssssssssiiddsss", $first_name, $middle_name, $last_name, $contact_number, $email, $handle, $sources, $source_other, $room, $check_in_date, $check_out_date, $check_in_time, $check_out_time, $guests, $additional_guest, $total_cost, $deposit_amount, $remaining_balance, $payment_method, $proof_of_payment);
...
?>
        ...
        <body>
                        ...
                        <nav class="nav">
                            <ul class="nav-list">
                                ...
                                foreach ($navLinks as $name => $link):
                                    $isActive = (basename($link) == $currentPage || ($currentPage == 'SubmitRoomReservation.php' && basename($link) == 'RoomsPage.php')) ? 'active' : '';
                                ?>
                                    <li class="nav-item">
            ...
            <main>
                <section class="section-hero section-hero--submit-room">
                    ...
                    <div class="reservation-body">
                        <div class="reservation-info">
                            ...
                            <ul class="reservation-list">
                                ...
                                <li class="reservation-item">
                                    <span class="reservation-label">Social</span>
                        ...
                        <div class="reservation-info">
                            ...
                            <ul class="reservation-list">
                                <li class="reservation-item">
                                    ...
                                    <span class="reservation-data"><?php echo htmlspecialchars($room, ENT_QUOTES, 'UTF-8'); ?></span>
                                </li>
                                <li class="reservation-item">
                                ...
                                <li class="reservation-item">
                                    <span class="reservation-label">Check-Out Date and Time</span>
                                    <span class="reservation-data"><?php echo date('F j, Y', strtotime(htmlspecialchars($check_out_date, ENT_QUOTES, 'UTF-8'))) . ($check_out_time ? ', at ' . date('h:i A', strtotime(htmlspecialchars($check_out_time, ENT_QUOTES, 'UTF-8'))) : ''); ?></span>
                                </li>
                                <li class="reservation-item">
                                ...
                                <li class="reservation-item">
                                    <span class="reservation-label">Additional Guests</span>
                                    <span class="reservation-data"><?php echo htmlspecialchars($additional_guest, ENT_QUOTES, 'UTF-8'); ?></span>
                                </li>
                            </ul>
                        </div>

                        <div class="reservation-info">
                            <h3 class="reservation-heading">Payment Summary</h3>
                            <ul class="reservation-list">
                                <li class="reservation-item">
                                    <span class="reservation-label">Total Cost</span>
                                    <span class="reservation-data">Php <?php echo number_format($total_cost, 2); ?></span>
                                </li>
                                <li class="reservation-item">
                                    <span class="reservation-label">Deposit Amount</span>
                                    <span class="reservation-data">Php <?php echo number_format($deposit_amount, 2); ?></span>
                                </li>
                                <li class="reservation-item">
                                    <span class="reservation-label">Remaining Balance</span>
                                    <span class="reservation-data">Php <?php echo number_format($remaining_balance, 2); ?></span>
                                </li>
                                <li class="reservation-item">
...
<?php
        ...
        die("Error inserting reservation data.");
    }
